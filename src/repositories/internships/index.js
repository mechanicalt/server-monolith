...
import Internship, { statusTypes } from 'models/Internship';
...
class InternshipRepo extends Repo {
  ...
  search = (searchText: string) => {
    ...
    const internshipsWithDocument = squel
      .select()
      .field('id')
      .field('name')
      .field('description')
    ...
    const query = squel
      ...
      .order(
        "ts_rank(p_search.document, to_tsquery('simple', ?))",
        false,
        querySearchText
      );
  ...
  mostRelevant = () => {
    const query = squel
      ...
      .where('internships.status = ?', statusTypes.ACTIVE)
